...
def input_parser(plan, input_args):
    ...
    for idx, network in enumerate(networks):
        ...
        command = "curl -s --connect-timeout 10 -X POST -H \"Content-Type: application/json\" -d '{\"jsonrpc\":\"2.0\",\"method\":\"eth_chainId\",\"params\":[],\"id\":1}' %s | jq -r '.result // \"0x0\"' | sed 's/^0x//' | xargs -I {} bash -c 'if [ -n \"{}\" ]; then echo \"ibase=16; {}\" | bc; else echo \"0\"; fi'" % rpc_url
